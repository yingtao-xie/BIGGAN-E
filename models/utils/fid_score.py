...
def calculate_fid_given_images_and_path1(images1, path, inception_path, splits=10):
    """ Calculates the FID of two paths. """
    # inception_path = check_or_download_inception(inception_path)
    f = np.load(path)
    m1, s1 = f['mu'][:], f['sigma'][:]
    ...
    d0 = images.shape[0]
    batch_size = d0//splits
    d0 = batch_size*splits
    ...
    with tf.Session(config=config) as sess:
        sess.run(tf.global_variables_initializer())
        fid_scores = []
        for i in tqdm(range(splits), desc="Calculate FID score"):
            sys.stdout.flush()
...
def calculate_fid_given_images_and_path(images1, path, inception_path, splits=10):
    """ Calculates the FID of two paths. """
    # inception_path = check_or_download_inception(inception_path)
    f = np.load(path)
    m2, s2 = f['mu'][:], f['sigma'][:]
    ...
    with tf.Session(config=config) as sess:
        sess.run(tf.global_variables_initializer())
        m1, s1 = calculate_activation_statistics(images, sess, batch_size=100, verbose=True)
        fid_value = calculate_frechet_distance(m1, s1, m2, s2)
